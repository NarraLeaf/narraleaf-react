...
/* Written under every frame of a scene's half. The scene root already carries its own position
   and size (a full-bleed absolute box from its class list), so the base contributes nothing but a
   stacking order and its visibility — anything more would fight the layout the scene owns.

   Deliberately no `will-change`. It is the obvious hint to give an element that is about to be
   animated, and on a scene root it is wrong: it promotes the whole stage to its own compositing
   layer, which rasterises on the device-pixel grid. The stage's geometry is fractional (the stage
   box is letterboxed to an aspect ratio, and every displayable's wrapper carries a half-pixel
   translate), so tearing the layer down at the end of the transition re-rasterises the content at
   its true sub-pixel position — the whole scene visibly snaps by under a pixel just as it settles.
   Measured: the last animated frame and the settled frame differed in 39% of their pixels by up to
   121/255 per channel with the hint, and by at most 3 (the residual fade) without it. Nothing is
   lost by dropping it: the driver writes opacity frame by frame as an inline style rather than
   handing the browser an animation, so there was never a promotion for the hint to bring forward.
...
export const SCENE_BASE_STYLE: CSSProps = {
    zIndex: 0,
    // Undoes {@link stageRetiredStyle} for a scene that is driven again — an undone jump can leave
    // a retired scene mounted, and it has to come back visible if a later transition uses it.
    visibility: "visible",
};
/**@internal */
export const INCOMING_SCENE_BASE_STYLE: CSSProps = {
    ...SCENE_BASE_STYLE,
    zIndex: 1,
};
 ...
 */
export function stageRetiredStyle(): CSSProps {
    return {
        ...stageSettledStyle(),
        visibility: "hidden",
    };
...
export class StageTransitionManager {
    ...
    public apply(transition: Transition, roles: StageTransitionRoles, resolve: VoidFunction): Timeline {
        ...
        controller.onComplete(() => {
            // The incoming scene goes back to painting normally; the outgoing one stops painting at
            // all until `scene:exit` takes it away. See {@link stageRetiredStyle}.
            this.settle(running, [[roles.to, stageSettledStyle()], [roles.from, stageRetiredStyle()]]);
            resolve();
            awaitable.resolve();
        });
        controller.onCanceled(() => {
            // No final frame was painted, so both halves keep a mid-animation pose unless they are
            // reset. Neither is retired: a cancelled jump is one that did not happen, and the
            // outgoing scene is the one the story is still in.
            this.settle(running, [[roles.from, stageSettledStyle()], [roles.to, stageSettledStyle()]]);
            timeline.abort();
        });
    ...
    private settle(running: RunningStageTransition, poses: [scene: Scene, style: CSSProps][]): void {
        running.stale = true;
