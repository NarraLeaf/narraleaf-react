...
export class Darkness extends ImageTransition<AnimationType> {
     ...
     */
    constructor(private prevBrightness: number, private targetBrightness: number, private duration: number, private easing?: TransformDefinitions.EasingDefinition) {
        super();
    }
    ...
    createTask(): TransitionTask<HTMLImageElement, AnimationType> {
        return {
            animations: [{
                ...
                start: this.prevBrightness,
                end: this.targetBrightness,
    ...
    copy(): Darkness {
        return new Darkness(this.prevBrightness, this.targetBrightness, this.duration, this.easing);
    }
}
