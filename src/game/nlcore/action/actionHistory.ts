...
export class ActionHistoryManager {
    ...
    public has(id: string): boolean {
        return this.history.some(entry => entry.id === id);
    }
     ...
     */
    public undoUntil(id: string): ActionHistory | null {
        let index = -1;
        ...
        for (let i = this.history.length - 1; i >= index; i--) {
            ...
            // Optional throughout: this is a log line on the undo path, and it must never be the
            // reason an undo fails.
            this.liveGame.getGameStateForce?.().logger?.debug(
                "ActionHistory", "Undoing", this.history[i].action.type
            );
    ...
    public undo(gameHistory: GameHistoryManager): ActionHistory | null {
